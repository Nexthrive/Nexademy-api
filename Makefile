...
db-start: ## start the database server
	...
	docker run --rm --name nexademy -v $(shell pwd)/testdata:/testdata \
		-v $(shell pwd)/testdata/postgres:/var/lib/postgresql/data \
		-e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=nexademy -d -p 5432:5432 postgres
...
testdata: ## populate the database with test data
	...
	@docker exec -it nexademy psql "$(APP_DSN)" -f /testdata/testdata.sql
